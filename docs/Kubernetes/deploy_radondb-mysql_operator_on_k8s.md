# 在 Kubernetes 上部署 RadonDB MySQL Operator
...
本教程主要演示如何在 Kubernetes 上部署 RadonDB MySQL 集群。
...
示例: 以下命令指定 release 名为 `demo` , 将创建一个名为 `demo-mysql-operator` 的部署(Deployment)，同时还会创建一个名为 `clusters.mysql.radondb.com` 的自定义资源(CRD)。
...
### 步骤三: 部署 RadonDB MySQL 集群

执行以下指令将以默认参数为自定义资源 `clusters.mysql.radondb.com` 创建一个实例，即创建 RadonDB MySQL 集群。您可以参照[配置](#配置)自定义集群部署参数。
...
执行如下指令将查看到 release 名为 `demo` 的部署及对应监控服务。
...
执行如下命令，将查看到名为 `clusters.mysql.radondb.com` 的自定义资源。
...
以默认部署为例，执行如下命令将查看到 release 名为 `sample` 的三节点 RadonDB MySQL 集群及用于访问节点的服务。
...
执行如下指令卸载 release 名为 `sample` RadonDB MySQL 集群实例。
...
| PodSpec.Labels            | 节点 pod [标签](https://kubernetes.io/zh/docs/concepts/overview/working-with-objects/labels/)                                                                           |                          |
| PodSpec.Annotations       | 节点 pod [注解](https://kubernetes.io/zh/docs/concepts/overview/working-with-objects/annotations/)                                                                      |                          |
| PodSpec.Affinity          | 节点 pod [亲和性](https://kubernetes.io/zh/docs/concepts/scheduling-eviction/assign-pod-node/#%E4%BA%B2%E5%92%8C%E6%80%A7%E4%B8%8E%E5%8F%8D%E4%BA%B2%E5%92%8C%E6%80%A7) |                          |
| PodSpec.PriorityClassName | 节点 pod [优先级](https://kubernetes.io/zh/docs/concepts/configuration/pod-priority-preemption/)对象名称                                                                |                          |
| PodSpec.Tolerations       | 节点 pod 可[容忍](https://kubernetes.io/zh/docs/concepts/scheduling-eviction/taint-and-toleration/)的污点列表                                                           |                          |
| PodSpec.SchedulerName     | 节点 pod 的[调度器](https://kubernetes.io/zh/docs/concepts/scheduling-eviction/kube-scheduler/)名称                                                                                                                                                   |                          |
